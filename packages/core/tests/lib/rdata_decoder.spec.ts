...
describe('rdata_to_string', () => {
    ...
    it('decodes AAAA without :: when no run >= 2 zero groups exists', () => {
        const { msg, rdata, rdataStart } = single_answer_message(TYPE_AAAA, new Uint8Array([
            0x20, 0x01, 0x00, 0x00, 0x0d, 0xb8, 0x00, 0x01,
            0x00, 0x00, 0x12, 0x34, 0x00, 0x00, 0x56, 0x78,
        ]));
        // Single zero groups don't collapse (RFC 5952 §4.2.2): pick the
        // longest run; here the longest run is exactly 2.
        const out = rdata_to_string(msg, TYPE_AAAA, rdata, rdataStart);
        expect(out).toMatch(/^2001:0:db8:1::1234:0:5678$|^2001:0:db8:1:0:1234::5678$|^2001:0:db8:1:0:1234:0:5678$/);
    });
    ...
    it('decodes NSEC3 with no salt and an empty type bitmap', () => {
        ...
        // Base32hex of [0x00, 0x01] = "0001" → "00040" (5 chars per 25 bits, padded with 0).
        // 16 bits → 4 base32 chars worth of bits, padded → 4 chars exact = "0004"? Let me recalc.
        // 16 bits / 5 = 3 chars + 1 leftover bit → pad to 20 bits → 4 chars: bits = 00000000 00000001 0000 → groups
        // 00000 00000 00000 10000 → 0, 0, 0, 16 → "000G".
        // Just verify by structure rather than the exact hash.
