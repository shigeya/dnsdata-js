...
describe("Domain name wire format conversion library", () => {
    ...
    describe("parse_domain_name (UP-002)", () => {
        ...
        it("rejects a label longer than 63 octets", () => {
            // 0x40 byte is a reserved prefix (b&0xC0 === 0x40), caught by
            // the reserved-prefix branch before the length check fires.
            // Use 0x40 to trigger the reserved-prefix branch; rely on
            // domain_name2wire (already exercised above) for the labels >
            // 63 case at the *encoder*.
            const msg = new Uint8Array([0x40, 0x00]);
            expect(() => parse_domain_name(msg, 0)).toThrow(DNSWireError);
        });
