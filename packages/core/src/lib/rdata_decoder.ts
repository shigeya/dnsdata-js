...
//
// Ports the dnsdata-go `wire/rdata.go` module (originated in
...
// NO dependency on dns_zone / dnssec_* — keeping the architectures
// of dnsdata-js and dnsdata-go in sync so future port-backs map
// file-to-file. The duplication of per-type RDATA layouts (also
// known to each handler's constructor and get_wire_body) is the
// deliberate cost of that sync; it is bounded to ~19 types and
