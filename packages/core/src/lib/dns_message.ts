...
//
// Ports the dnsdata-go `wire/message.go` module (originated in
// dnsdata-go v0.1.0; tracked here as UP-002 / shigeya/dnsdata-js#6).
//
// Lives in the wire layer alongside dns_wire.ts with NO dependency
// on dns_zone / dnssec_*. Higher-level adapters that lift RawRR
// into ResourceRecord live in their callers (mirroring the Go side's
// resolver/doh.Client.Resolve approach). Keeping the architectures
// in sync makes future cross-language port-backs mechanical — every
// Go-side change to wire/message.go has a single corresponding TS
// file to update.
